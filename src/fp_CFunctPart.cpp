...
double CFunctPart::min()
{
    if (a2 == 0) {
        if (a1 == 0)
            ...
            return std::min(lBorder * a1, rBorder * a1) + a0;  // should not occur
        }
    } else {
        double xMin = -a1 / (2 * a2);
        if ((lBorder <= xMin) & (xMin <= rBorder)) {
            ...
            return std::min(lBorder * (a2 * lBorder + a1), rBorder * (a2 * rBorder + a1)) + a0;
        }
    }
}
double CFunctPart::mean()
{
    ...
    if (a2 != 0) {
        double xMin = -a1 / (2 * a2);
        xMin = std::min(std::max(lBorder, xMin), rBorder);
        return xMin;
    } else {
        if (a1 >= 0)
            return lBorder;
...
void CFunctPart::updateRoots(double a0_)
{
    ...
    rac1 = std::max(rac1, lBorder);
    rac2 = std::min(rac2, rBorder);
    ...
    // cout << "Ori : " << this->origine << ", stat: " << this->racStatus << ", rac1: " << this->rac1 << ", rac2: " <<
    // this->rac2 << "poly : " << a2 << "x^2 + " << a1 << "x + " << a0 << "- " << a0_ << "[" << lBorder << "," <<
    // rBorder << "]" <<"\n";
}
